...
export default function RelatoriosPage() {
  ...
  const summaryCards = [
    { label: "Dias trabalhados", value: daysWorked, icon: CalendarDays, color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Horas trabalhadas", value: formatHours(totalHours), icon: Clock, color: "text-green-600", bg: "bg-green-50" },
    { label: "Horas extras", value: formatHours(totalOvertime), icon: TrendingUp, color: "text-amber-700", bg: "bg-amber-50" },
    { label: "Valor extras", value: formatCurrency(totalOvertimeValue), icon: DollarSign, color: "text-purple-600", bg: "bg-purple-50" },
  ];

  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Relatórios Mensais</h1>
        <p className="text-sm text-gray-500 mt-0.5">Gere e exporte relatórios de ponto por funcionário.</p>
      </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select
              ...
              className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-green-500 min-w-[180px] transition-all"
            >
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">Mês</label>
            <select
              value={month}
          ...
          <div>
            ...
            <input
              ...
              className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-green-500 w-24 transition-all"
            />
          </div>
          <button
          ...
          {workdays.length > 0 && (
            <button
              onClick={handlePDF}
              className="inline-flex items-center gap-2 bg-amber-800 hover:bg-amber-900 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors"
            >
              <Download size={15} />
              Gerar PDF
            </button>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {summaryCards.map((c) => {
              const Icon = c.icon;
              return (
                <div key={c.label} className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-3">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">{daysWorked} dias completos</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {workdays.map((d) => (
                    <tr key={d.id} className={`hover:bg-gray-50/50 transition-colors ${d.bank_balance < 0 ? "bg-red-50/30" : ""}`}>
                      ...
                      <td className="px-3 py-2.5 text-amber-700 tabular-nums">
                        {d.overtime_hours > 0 ? formatHours(d.overtime_hours) : "--"}
                      </td>
                      <td className={`px-3 py-2.5 font-semibold tabular-nums ${d.bank_balance >= 0 ? "text-green-700" : "text-red-600"}`}>
                ...
                <tfoot className="bg-green-50 border-t-2 border-green-100">
                  <tr>
                    <td colSpan={5} className="px-3 py-3 font-bold text-gray-800">
                      TOTAL ({daysWorked} dias)
                    </td>
                    <td className="px-3 py-3 font-bold text-gray-800 tabular-nums">{formatHours(totalHours)}</td>
                    <td className="px-3 py-3 font-bold text-amber-700 tabular-nums">{formatHours(totalOvertime)}</td>
                    <td className={`px-3 py-3 font-bold tabular-nums ${totalBank >= 0 ? "text-green-700" : "text-red-600"}`}>
      ...
      {!loading && workdays.length === 0 && selectedEmployee && (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-500 font-medium">Nenhum registro no período</p>
          <p className="text-gray-400 text-sm mt-1">Tente outro mês ou funcionário.</p>
        </div>
      )}
    </div>
  );
